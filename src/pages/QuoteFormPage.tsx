...
const itemSchema = z.object({ product_id: z.number().nullable().optional(), item_name: z.string().min(1, 'Informe o item.'), item_description: z.string().nullable().optional(), quantity: z.number().positive('Quantidade maior que zero.'), unit_price: z.number().min(0, 'Preço inválido.') });
const schema = z.object({ client_id: z.number().positive('Selecione o cliente.'), status: z.enum(['draft', 'sent', 'approved', 'rejected']), discount_type: z.enum(['fixed', 'percentage']), discount_value: z.number().min(0), notes: z.string(), valid_until: z.string().min(1, 'Informe a validade.'), items: z.array(itemSchema).min(1, 'Adicione pelo menos um item.') }).refine((v) => v.discount_type !== 'percentage' || v.discount_value <= 100, { path: ['discount_value'], message: 'Máximo de 100%.' });
